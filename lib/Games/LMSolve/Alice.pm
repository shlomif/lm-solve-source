...
INPUT_LOOP: while ( $read_line->() )
        ...
        while ( length($line) > 0 )
        {
            ...
            if ( $line =~ /\S/ )
            {
                if ( $line =~ /^\[([^\]]*)\]/ )
                {
                    ...
                    my @flag_flags =
                        ( grep { exists( $cell_flags{$_} ) } @flags );
                    my @unknown_flags = (
                        grep {
                                   ( !exists( $cell_dirs{$_} ) )
                                && ( !exists( $cell_flags{$_} ) )
                    ...
                    {
                        $gen_exception->( "Unknown Flags on Cell ("
                                . join( ",", @unknown_flags )
                                . ")" );
                    }
                    $board[$y][$x] = {
    ...
    {
        $gen_exception->(
            "Input Terminated Prematurely after reading y=$y x=$x");
    }
...
sub check_if_final_state
{
    ...
    return
        exists(
        $self->{'board'}->[ $coords->[1] ][ $coords->[0] ]->{'flags'}->{'GOAL'}
        );
}
...
sub enumerate_moves
{
    ...
    return
        keys(
        %{ $self->{'board'}->[ $coords->[1] ][ $coords->[0] ]->{'dirs'} } );
}
...
sub perform_move
{
    ...
    my $new_cell =
        $self->{'board'}->[ $new_coords[1] ][ $new_coords[0] ]->{'flags'};

    # Check if we are out of the bounds of the board.
    if (   ( $new_coords[0] < 0 )
        || ( $new_coords[0] >= $self->{'width'} )
        || ( $new_coords[1] < 0 )
        || ( $new_coords[1] >= $self->{'height'} )
